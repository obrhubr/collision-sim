...
std::vector<Objects::Object> io::readObjects() {
    ...
    for (std::string line; getline(infile, line); ) {
        if (line.at(0) == 'B') {
            ...
            tempObj.set(objData[0], objData[1], objData[2], objData[3], objData[4], objData[5], objData[6], objData[7], objData[8]);
...
std::string io::convert(Physics::Physics physics) {
    ...
    for (int i = 0; i < objectlist.size(); i++) {
        content.append(std::to_string(objectlist[i].getPos()[0]));
        content.append(",");
        content.append(std::to_string(objectlist[i].getPos()[1]));
        content.append(",");
    };
    content.append("\n");
    return content;
